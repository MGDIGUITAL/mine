...
function setQuoteListeners() {
  const forms = document.querySelectorAll('form[action="/tier/update"]')

  for (const form of forms) {
    if (form.dataset.listenersSet) continue
    form.dataset.listenersSet = true

    form.addEventListener('submit', async (e) => {
      e.preventDefault()

      const popup = form.closest('.store-quote-popup')
      ...
      if (popup) closePopup(popup, true, true)
      ...
      if (response.ok) {
        ...
        newToast(data.error || __('Failed to update subscription'), 'error')
      }
    })
  }
}
...
function setSlider(container) {
  ...
  openLightboxButton?.addEventListener('click', () => {
    ...
    const iframes = swiperEl.querySelectorAll('iframe.slide-frame')

    // Unload iframes in all slides
  ...
  swiperEl.swiper.on('slideChangeTransitionEnd', () => {
    ...
    // Load iframe for active slide if it hasn't been loaded yet
    const
      activeSlide = swiperEl.swiper.slides[activeIndex],
      iframe = activeSlide?.querySelector('iframe.slide-frame')
    ...
    // Unload iframes in all inactive slides
    swiperEl.swiper.slides.forEach((slide, index) => {
      if (index !== activeIndex) {
        const iframe = slide.querySelector('iframe.slide-frame')
  ...
  // Load iframe for initial slide
  const
    initialSlide = swiperEl.swiper.slides[activeIndex],
    initialFrame = initialSlide?.querySelector('iframe.slide-frame')

  if (initialFrame && initialFrame.dataset.src) {
    initialFrame.classList.remove('iframe-loaded')

    initialFrame.src = initialFrame.dataset.src

    initialFrame.addEventListener('load', () => {
      initialFrame.classList.add('iframe-loaded')
    })
  }
}
...
async function openMediaLightbox(sourceContainer, index = 0) {
  ...
  for (let i = 0; i < sourceSlides.length; i++) {
    ...
    if (youtubeFrame) {
      if (i === index && youtubeFrame.dataset.src) {
        youtubeFrame.classList.remove('iframe-loaded')

        youtubeFrame.src = youtubeFrame.dataset.src

        youtubeFrame.addEventListener('load', () => {
          youtubeFrame.classList.add('iframe-loaded')
        })
      } else {
        youtubeFrame.src = ''
  ...
  const onSlideChangeTransitionEnd = () => {
    ...
    // Unload iframes in all inactive slides
    swiperEl.swiper.slides.forEach((slide, slideIndex) => {
      if (slideIndex !== activeIndex) {
        const inactiveFrame = slide.querySelector('iframe.slide-frame')

        if (inactiveFrame && inactiveFrame.src) {
          inactiveFrame.src = ''
        }

        if (inactiveFrame) {
          inactiveFrame.classList.remove('iframe-loaded')
        }
      }
    })
